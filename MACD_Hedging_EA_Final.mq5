...
#property description "MACD-based Expert Advisor with Hedging Logic - Final Version"
#property strict
...
string currentSymbol = "";                                 // Changed from 'symbol' to avoid conflict
double symbolPoint = 0.0;                                 // Changed from 'point'
double symbolTickSize = 0.0;                             // Changed from 'tickSize'
double symbolTickValue = 0.0;                            // Changed from 'tickValue'
int symbolDigits = 0;                                     // Changed from 'digits'
...
int OnInit()
{
    ...
    symbolTickValue = SymbolInfoDouble(currentSymbol, SYMBOL_TRADE_TICK_VALUE);
    ...
    macdHandle = iMACD(currentSymbol, InpTimeframe, InpMACDFastEMA, InpMACDSlowEMA, InpMACDSignalSMA, InpMACDPrice);
    ...
    trade.SetMarginMode();
    if(!trade.SetTypeFillingBySymbol(currentSymbol))
    {
        Print("Warning: Could not set filling type for symbol ", currentSymbol);
    }
    ...
    for(int i = 0; i < ArraySize(activeTrades); i++)
    {
        ...
        activeTrades[i].level = 0;
        activeTrades[i].lot = 0.0;
        activeTrades[i].openPrice = 0.0;
        activeTrades[i].type = POSITION_TYPE_BUY;
    ...
    Print("MACD Hedging EA initialized successfully for ", currentSymbol);
...
void OnTick()
{
    ...
    datetime currentBarTime = iTime(currentSymbol, InpTimeframe, 0);
...
bool OpenTrade(ENUM_ORDER_TYPE orderType, double lot, int level)
{
    ...
    if(orderType == ORDER_TYPE_BUY)
        result = trade.Buy(lot, currentSymbol, price, sl, tp, comment);
    else if(orderType == ORDER_TYPE_SELL)
        result = trade.Sell(lot, currentSymbol, price, sl, tp, comment);
...
void CheckHedgingOpportunities()
{
    ...
    for(int i = 0; i < totalActiveTrades; i++)
    {
        ...
        {
            pipsDiff = (openPrice - currentPrice) / symbolPoint;
        }
        else if(activeTrades[i].type == POSITION_TYPE_SELL)
        {
            pipsDiff = (currentPrice - openPrice) / symbolPoint;
        }
        else
        {
...
double GetTotalProfit()
{
    ...
    for(int i = 0; i < totalPositions; i++)
    {
        ...
        {
            if(positionInfo.Symbol() == currentSymbol && positionInfo.Magic() == 12345)
            {
...
bool CheckRiskManagement()
{
    ...
    if(ask <= 0.0 || bid <= 0.0 || symbolPoint <= 0.0)
        return false;
        
    double spread = (ask - bid) / symbolPoint;
...
void UpdateDashboard()
{
    ...
    if(ObjectFind(0, prefix + "Value_0") >= 0)
        ObjectSetString(0, prefix + "Value_0", OBJPROP_TEXT, currentSymbol);
